...
if (isset($_REQUEST['query'])) {
    ...
    $sql="SELECT firstName, lastName FROM MyDB.users WHERE ((firstName LIKE '" . $query .  "%' OR lastName LIKE '" . $query ."%') AND (email IN (SELECT emailTo FROM MyDB.friendships WHERE (emailFrom='" . $loggedInUser . "' AND status='accepted')) OR email IN (SELECT emailFrom FROM MyDB.friendships WHERE ( emailTo='". $loggedInUser . "' AND status='accepted')) OR email IN (SELECT emailTo FROM MyDB.friendships WHERE (emailFrom IN " .$friendStr . " AND status='accepted') OR email IN (SELECT emailFrom FROM MyDB.friendships WHERE(emailTo IN " . $friendStr . " AND status='accepted')))) AND email!='" . $loggedInUser . "')";
