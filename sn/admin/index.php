...
<body>
  ...
  echo "<table class='table table-stripped table-bordered'>
    ...
    foreach ($pdo->query($userInfo) as $row)  {
        ...
        echo "<td> <a class='table-btn btn btn-success' href='editUserView.php?email=".$row["email"]."'><i class='fa fa-pencil' aria-hidden='true'></i> Edit </a>";
        echo "<br> <a class='table-btn btn btn-danger' href='deleteUser.php?email=".$row["email"]."'> <i class='fa fa-trash' aria-hidden='true'></i> Delete </td> </a>";
        echo "</tr>";
    }
